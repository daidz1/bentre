...
import changeOrgDialogCss from './components/change-org-dialog.css';
import dialogCss from './components/dialog.css';
import paginationCss from './components/pagination.css';
import ratingCss from './components/rating.css';
import uploadCss from './components/upload.css';

window.Vaadin = window.Vaadin || {};
window.Vaadin.Flow = window.Vaadin.Flow || {};
window.Vaadin.Flow['_vaadintheme_site_globalCss'] = window.Vaadin.Flow['_vaadintheme_site_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_site_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_site_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_site_globalCss'].push(target);
  }
  if (!document['_vaadintheme_site_componentCss']) {
    registerStyles(
      'change-org-dialog',
      css`
        ${unsafeCSS(changeOrgDialogCss.toString())}
      `
    );
    registerStyles(
      'dialog',
    ...
    registerStyles(
      'pagination',
      css`
        ${unsafeCSS(paginationCss.toString())}
      `
    );
    registerStyles(
      'rating',
      css`
        ${unsafeCSS(ratingCss.toString())}
      `
    );
    registerStyles(
      'upload',
    ...
    document['_vaadintheme_site_componentCss'] = true;
  }
