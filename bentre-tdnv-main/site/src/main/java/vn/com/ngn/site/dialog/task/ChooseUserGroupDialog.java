...
import vn.com.ngn.site.model.taskcreate.UserTemplateGridModel;
...
public class ChooseUserGroupDialog extends DialogTemplate{
	...
	private Grid<UserTemplateGridModel> grid = new Grid<UserTemplateGridModel>();
	private List<UserTemplateGridModel> listData = new ArrayList<UserTemplateGridModel>();
	...
	public void buildLayout() {
		...
		caption.setText("Chọn nhóm giao nhiệm vụ");
	...
	public void configComponent() {
		...
		btnAdd.addClickListener(e->{
			EditUserGroupDialog dialog = new EditUserGroupDialog(null);
	...
	private void buildGrid() {
		grid.addComponentColumn(UserTemplateGridModel::getName).setHeader("Tên nhóm").setWidth("100px");
		grid.addComponentColumn(UserTemplateGridModel::getDescription).setHeader("Mô tả").setWidth("100px");
		grid.addComponentColumn(UserTemplateGridModel::getUserAssign).setHeader("Cán bộ xử lý");
		grid.addComponentColumn(UserTemplateGridModel::getUserSupport).setHeader("Cán bộ hỗ trợ"); 
		grid.addComponentColumn(UserTemplateGridModel::getAction).setHeader("Hành động");
	...
	private void loadData() throws IOException {
		...
		if(jsonResponse.get("status").getAsInt()==200) {
			...
			for(JsonElement jsonEle : jsonTaskList) {
				...
				UserTemplateGridModel modelGrid = new UserTemplateGridModel();
				modelGrid.setName(htmlName);
				modelGrid.setDescription(htmlDescription);
				modelGrid.setUserAssign(htmlUserAssign);
				modelGrid.setUserSupport(menuFollower);
