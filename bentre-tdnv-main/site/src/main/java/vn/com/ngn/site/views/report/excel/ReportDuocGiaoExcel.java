...
public class ReportDuocGiaoExcel extends ExcelExport{
	private String template = templatePath+"duocgiao.xlsx";
	...
	{	
		...
		return getStreamResource("Nhiem vu duoc giao-"+new Date().getTime()+".xlsx", byteOutputStream.toByteArray());
	}
	
	public void createSheet() throws XPathExpressionException {
		XSSFSheet sheetSum = workbook.getSheet("duocgiao");
		...
		for(ReportModel model : listData) {
			...
			cell = row.getCell(2);
			cell.setCellValue(model.getNguoiGiao());
			cell = row.getCell(3);
