...
public class MultiTaskCreateOrgLayout extends VerticalLayout implements LayoutInterface{
	private int count = 0;
	...
	private LinkedList<TaskBlockForMultiCreateOrgLayout> listTaskLayout = new LinkedList<TaskBlockForMultiCreateOrgLayout>();

	public MultiTaskCreateOrgLayout() {
		buildLayout();
		configComponent();
	}
	...
	public void configComponent() {
		...
		btnCreateTask.addClickListener(e->{
			if(listTaskLayout.size()>0) {
				...
				for(TaskBlockForMultiCreateOrgLayout modelTaskLayout : listTaskLayout) {
				...
				if(isValid) {
					...
					List<TaskBlockForMultiCreateOrgLayout> listToDelete = new ArrayList<TaskBlockForMultiCreateOrgLayout>();
					for(TaskBlockForMultiCreateOrgLayout modelTaskLayout : listTaskLayout) {
						TaskDataForMultiCreateOrgModel modelData = modelTaskLayout.getData();
						
						String taskTitle = modelData.getModelTaskInfo().getTitle();
						String assigneeName = modelData.getModelOrgAssignee().getOrgName();
						...
						try {
							...
							JsonObject jsonResponse = TaskServiceUtil.createOrgTask(null,null,modelTaskInfo,modelUserOwner,null, modelData.getModelOrgAssignee(), modelData.getListOrgSupport());
							...
							if(jsonResponse.get("status").getAsInt()==201) {
								...
								if(!cbType.getValue()) {
									...
									NotificationUtil.showNotifi("Giao nhiệm vụ cho cơ quan/đơn vị "+assigneeName+" thành công", NotificationTypeEnum.SUCCESS);
								}
								...
								if(!cbType.getValue()) {
									...
									NotificationUtil.showNotifi("Có lỗi xảy ra khi giao nhiệm vụ cơ quan/đơn vị "+assigneeName+" vui lòng thử lại sau!", NotificationTypeEnum.ERROR);
								}
							}
						} catch (Exception e2) {
							e2.printStackTrace();
					...
					if(isAllGood) {
						...
						mapParam.put("assignmentType", TaskAssignmentTypeEnum.ORGANIZATION.getKey());
	...
	private void addNewTask(TaskDataForMultiCreateOrgModel modelDuplicateData) {
		TaskBlockForMultiCreateOrgLayout modelLayoutTask = new TaskBlockForMultiCreateOrgLayout();
	...
	private void reloadTaskList() {
		...
		for(TaskBlockForMultiCreateOrgLayout model : listTaskLayout) {
